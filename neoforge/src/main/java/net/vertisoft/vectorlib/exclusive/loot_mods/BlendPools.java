...
public class BlendPools implements IGlobalLootModifier
{
    private final ResourceKey<LootTable> target_table;
    private final ResourceKey<LootTable> merging_table;
    private final float replacement_chance;

    public static final MapCodec<BlendPools> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(
                ResourceKey.codec(Registries.LOOT_TABLE).fieldOf("target_table").forGetter(BlendPools::getTargetTable),
                ResourceKey.codec(Registries.LOOT_TABLE).fieldOf("merging_table").forGetter(BlendPools::getMergedTable),
                Codec.FLOAT.fieldOf("replace_chance").forGetter(BlendPools::getReplaceChance)
        ).apply(instance, BlendPools::new);
    });

    public BlendPools(ResourceKey<LootTable> target_table, ResourceKey<LootTable> merging_table, float replacement_chance)
    {
        this.target_table = target_table;
        this.merging_table = merging_table;
        this.replacement_chance = replacement_chance;
    }

    public MapCodec<? extends IGlobalLootModifier> codec() { return NFLootModSet.BLEND_POOLS.get(); }
    ...
    public float getReplaceChance() { return this.replacement_chance; }
    ...
    protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context)
    {
        ...
        if (mergerTable.isPresent())
        {
            ...
            if (targeter.isPresent())
            {
                ...
                {
                    VectorLib.LOGGER.error("LootMod -> BlendPools: Provided Loot Tables must have one pool each. Falling back to default drops.");
                }
                else
                {
                    ...
                    if (context.getRandom().nextFloat() >= this.replacement_chance) return mergeTarget;
                }
            }
        }
        return generatedLoot;
