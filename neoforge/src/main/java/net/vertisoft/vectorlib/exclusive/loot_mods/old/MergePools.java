...
/** An older variant of AddExtraPools, just kept for convenience. Best not to use as it'll ignore modded additions.  */
@Experimental
public class MergePools implements IGlobalLootModifier
{
    private final ResourceKey<LootTable> target_table;
    private final ResourceKey<LootTable> merging_table;

    public static final MapCodec<MergePools> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
        return instance.group(
                ResourceKey.codec(Registries.LOOT_TABLE).fieldOf("target_table").forGetter(MergePools::getTargetTable),
                ResourceKey.codec(Registries.LOOT_TABLE).fieldOf("merging_table").forGetter(MergePools::getMergedTable)
        ).apply(instance, MergePools::new);
    });

    public MergePools(ResourceKey<LootTable> target_table, ResourceKey<LootTable> merging_table)
    {
    ...
    protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context)
    {
        ...
        if (mergerTable.isPresent())
        {
            ...
            if (targeter.isPresent())
            {
                ...
                for (LootPool pooler : combo) builder.pools.add(pooler);
    ...
    public MapCodec<? extends IGlobalLootModifier> codec() { return NFLootModSet.MERGE_POOLS.get(); }
}
