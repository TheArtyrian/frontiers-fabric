...
public class NFVegetationGeneration
{
    ...
    public static void register(BootstrapContext<BiomeModifier> context)
    {
        ...
        context.register(ADD_FUNGAL_DAFFODIL, new BiomeModifiers.AddFeaturesBiomeModifier(
                biomes.getOrThrow(FRTags.Biomes.GENERATES_FUNGAL_DAFFODIL),
                HolderSet.direct(placedFeatures.getOrThrow(FRFeaturesPlaced.FUNGAL_DAFFODIL_PLACED_KEY)),
                GenerationStep.Decoration.VEGETAL_DECORATION
        ));

        context.register(ADD_SNOW_DAHLIA, new BiomeModifiers.AddFeaturesBiomeModifier(
                biomes.getOrThrow(FRTags.Biomes.GENERATES_SNOW_DAHLIA),
                HolderSet.direct(placedFeatures.getOrThrow(FRFeaturesPlaced.SNOW_DAHLIA_PLACED_KEY)),
                GenerationStep.Decoration.VEGETAL_DECORATION
        ));

        context.register(ADD_CRIMCONE, new BiomeModifiers.AddFeaturesBiomeModifier(
                biomes.getOrThrow(FRTags.Biomes.GENERATES_CRIMCONE),
                HolderSet.direct(placedFeatures.getOrThrow(FRFeaturesPlaced.CRIMCONE_PLACED_KEY)),
                GenerationStep.Decoration.VEGETAL_DECORATION
        ));

        context.register(ADD_EXPERIWINKLE, new BiomeModifiers.AddFeaturesBiomeModifier(
                biomes.getOrThrow(FRTags.Biomes.GENERATES_EXPERIWINKLE),
                HolderSet.direct(placedFeatures.getOrThrow(FRFeaturesPlaced.EXPERIWINKLE_PLACED_KEY)),
                GenerationStep.Decoration.VEGETAL_DECORATION
        ));
    }
}
