...
public class FREntityLootTableProvider extends SimpleFabricLootTableProvider
{
    private CompletableFuture<HolderLookup.Provider> registryLookup;

    public FREntityLootTableProvider(FabricDataOutput output, CompletableFuture<HolderLookup.Provider> registryLookup)
    {
    ...
    private void createLivingEntity(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> lootTableBiConsumer, HolderLookup.Provider lookup)
    {
        lootTableBiConsumer.accept(
                FREntity.CRAWLER.get().getDefaultLootTable(),
                LootTable.lootTable()
                        .withPool(
                                LootPool.lootPool()
                        ...
                        .withPool(
                                LootPool.lootPool()
                                        .setRolls(ConstantValue.exactly(1.0F))
                                        .add(
                                                LootItem.lootTableItem(FRItems.SOUL.get())
        ...
        lootTableBiConsumer.accept(
                FREntity.CROW.get().getDefaultLootTable(),
                LootTable.lootTable()
                        .withPool(
                                LootPool.lootPool()
                                        .setRolls(ConstantValue.exactly(1.0F))
        ...
        lootTableBiConsumer.accept(
                FREntity.PUMPKIN_GOLEM.get().getDefaultLootTable(),
                LootTable.lootTable()
                        .withPool(
                                LootPool.lootPool()
                                        .setRolls(ConstantValue.exactly(1.0F))
                                        .add(
                                                LootItem.lootTableItem(FRItems.SPIRIT_CANDLE.get())
                                                        .apply(SetItemCountFunction.setCount(ConstantValue.exactly(1.0F)))
                                        )
                        )
        );
        lootTableBiConsumer.accept(
                FREntity.JUNGLE_SPIDER.get().getDefaultLootTable(),
                LootTable.lootTable()
                        .withPool(
                                LootPool.lootPool()
                                        .setRolls(ConstantValue.exactly(1.0F))
    ...
    private void createMisc(BiConsumer<ResourceKey<LootTable>, LootTable.Builder> lootTableBiConsumer, HolderLookup.Provider lookup)
    {
        // Vex Ragekill
        lootTableBiConsumer.accept(
                FRLootTables.VEX_RAGE,
                LootTable.lootTable()
                        .withPool(
                                LootPool.lootPool()
                                        .setRolls(ConstantValue.exactly(1.0F))
                                        .add(
                                                LootItem.lootTableItem(FRItems.INCENSE.get())
                                                        .apply(SetItemCountFunction.setCount(UniformGenerator.between(0.0F, 2.0F)))
                                                        .apply(EnchantedCountIncreaseFunction.lootingMultiplier(lookup, UniformGenerator.between(0.0F, 1.0F)))
                                        )
                        )
        ...
        // Shulker Bullet
        lootTableBiConsumer.accept(
                FRLootTables.SHULKER_BULLET,
                LootTable.lootTable()
                        .withPool(
                                LootPool.lootPool()
                                        .setRolls(ConstantValue.exactly(1.0F))
                                        .add(
                                                LootItem.lootTableItem(FRItems.SHULKER_RESIDUE.get())
                                                        .apply(SetItemCountFunction.setCount(UniformGenerator.between(0.0F, 1.0F)))
                                                        .apply(EnchantedCountIncreaseFunction.lootingMultiplier(lookup, UniformGenerator.between(0.0F, 1.0F)))
                                        )
                        )
