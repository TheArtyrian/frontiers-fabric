...
import net.artyrian.frontiers.block.custom.models.StrayModelBlock;
import net.artyrian.frontiers.block.entity.entity_model.SkeletonModelBlockEntity;
import net.artyrian.frontiers.block.entity.entity_model.StrayModelBlockEntity;
...
public class StrayModelBlockEntityRenderer implements BlockEntityRenderer<StrayModelBlockEntity>
{
    private final ModelPart body;
    private static final Identifier TEXTURE = Identifier.of(Frontiers.MOD_ID, "textures/entity/mob_model/stray_model.png");
    private static final RenderLayer LAYER = RenderLayer.getEntityCutoutNoCull(TEXTURE);

    public StrayModelBlockEntityRenderer(BlockEntityRendererFactory.Context context)
    {
        this.body = getModel().createModel();
    }

    @Override
    public void render(StrayModelBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay)
    {
        matrices.push();

        BlockState blockState = entity.getCachedState();
        float h = 0.0F;
        if (blockState.getBlock() instanceof StrayModelBlock) h = RotationPropertyHelper.toDegrees(blockState.get(StrayModelBlock.ROTATION));
