...
import net.artyrian.frontiers.block.entity.entity_model.WitherSkeletonModelBlockEntity;
...
public class WitherSkeletonModelBlockEntityRenderer implements BlockEntityRenderer<WitherSkeletonModelBlockEntity>
{
    ...
    private static final Identifier TEXTURE = Identifier.of(Frontiers.MOD_ID, "textures/entity/mob_model/wither_skeleton_model.png");
    private static final RenderLayer LAYER = RenderLayer.getEntityCutoutNoCull(TEXTURE);

    public WitherSkeletonModelBlockEntityRenderer(BlockEntityRendererFactory.Context context)
    {
        this.body = getModel().createModel();
    }

    @Override
    public void render(WitherSkeletonModelBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay)
    {
        matrices.push();

        BlockState blockState = entity.getCachedState();
        float h = 0.0F;
        if (blockState.getBlock() instanceof WitherSkeletonModelBlock) h = RotationPropertyHelper.toDegrees(blockState.get(WitherSkeletonModelBlock.ROTATION));

        matrices.translate(0.5F, 0.0F, 0.5F);
        matrices.scale(1.2F, 1.2F, 1.2F);
        matrices.translate(0.0F, 1.489F, 0.0F);
