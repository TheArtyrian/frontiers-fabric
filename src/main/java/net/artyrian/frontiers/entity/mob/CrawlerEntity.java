...
import net.artyrian.frontiers.entity.ai.crawler.CrawlerIgniteGoal;
...
public class CrawlerEntity extends HostileEntity implements SkinOverlayOwner
{
    private static final TrackedData<Integer> FUSE_SPEED = DataTracker.registerData(CrawlerEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Boolean> CHARGED = DataTracker.registerData(CrawlerEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<Boolean> IGNITED = DataTracker.registerData(CrawlerEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    public void tick()
    {
        if (this.isAlive())
        {
            ...
            {
                this.playSound(ModSounds.CRAWLER_PRIMED, 1.0F, 1.0F);
    ...
    private void explode() {
        if (!this.getWorld().isClient)
        {
            float f = this.shouldRenderOverlay() ? 1.5F : 1.0F;
            this.dead = true;
            this.getWorld().createExplosion(this, this.getX(), this.getY(), this.getZ(), (float)this.explosionRadius * f, this.isOnFire(), World.ExplosionSourceType.MOB);
