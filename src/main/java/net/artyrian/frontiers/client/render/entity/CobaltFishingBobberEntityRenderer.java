...
public class CobaltFishingBobberEntityRenderer extends FishingBobberEntityRenderer
{
    private static final Identifier TEXTURE = Identifier.of(Frontiers.MOD_ID, "textures/entity/cobalt_fishing_hook.png");
    private static final RenderLayer LAYER = RenderLayer.getEntityCutout(TEXTURE);

    public CobaltFishingBobberEntityRenderer(EntityRendererFactory.Context context)
    {
        super(context);
    }
    ...
    private Vec3d getHandPos(PlayerEntity player, float f, float tickDelta) {
        ...
        if (!itemStack.isOf(ModItem.COBALT_FISHING_ROD)) {
            i = -i;
        }
    ...
    private static void renderFishingLine(float x, float y, float z, VertexConsumer buffer, MatrixStack.Entry matrices, float segmentStart, float segmentEnd) {
        ...
        j /= l;
        k /= l;
        buffer.vertex(matrices, f, g, h).color(Colors.BLUE).normal(matrices, i, j, k);
    }
}
